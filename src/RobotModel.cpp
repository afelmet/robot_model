...
{
    for(const std::string &link_name : visted_links_names)
    {
...
{
     for(const std::pair<std::string, std::string> &collision : disabled_collision_pair)
     {
        srdf::Model::CollisionPair disabled_pair;
        disabled_pair.link1_ = collision.first;
        disabled_pair.link2_ = collision.second;
...
bool RobotModel::getJointLimits(std::vector< double > &lower_limits, std::vector< double > &upper_limits)
{
    ...
    if(!getPlanningGroupJointInformation(planning_group_name_, planning_groups_joints))
        return false;
    ...
    for(auto it = planning_groups_joints.begin(); it != planning_groups_joints.end(); it++)
    {
        lower_limits.push_back(it->second.limits->lower)  ;
        upper_limits.push_back(it->second.limits->upper)  ;	
    }

    assert(lower_limits.size() == planning_groups_joints.size());
...
bool RobotModel::getJointLimits(base::JointLimits &limits)
{
    ...
    assert(limits.size() == planning_groups_joints.size());
    return true;
}
...
//#define COMPUTEJACOBAIN_LOG
    ...
    for(const KDL::Segment &segment : kdl_chain.segments)
    {
        joint_name = segment.getJoint().getName();
        joint_type = segment.getJoint().getType();
                                  ...
                                  KDL::Chain &kdl_chain) const
{
    ...
    for(const srdf::Model::Group &group : srdf_groups)
    {
        if( planning_group_name.compare(group.name_) == 0 )
        {
    ...
    for(const std::pair<std::string, std::string> &chain : planning_group.chains_)
    {
        base_link = chain.first;
        tip_link = chain.second;
    }
                                                  ...
                                                  std::vector< std::string> &planning_group_joints_name)
{
    ...
    for(const KDL::Segment &segment : kdl_chain.segments)
    {
        ...
        if(! (segment.getJoint().getType()==KDL::Joint::None) )
        {
            ...
            planning_group_joints_name.push_back(joint_name);
        }
    }
...
bool RobotModel::getPlanningGroupJointInformation(const std::vector<std::string> &planning_groups_name, base::samples::Joints &planning_groups_joints)
{
    ...
    for(const std::string &group : planning_groups_name)
    {
        // get kdl_chain, base and tip frame
        KDL::Chain kdl_chain;
        std::string base_frame, tip_frame ;
        if(!getPlanningGroup(group, base_frame, tip_frame, kdl_chain))
            return false;
        ...
        for(const KDL::Segment &segment : kdl_chain.segments)
        {
            ...
            if(! (segment.getJoint().getType()==KDL::Joint::None) )
            {
                ...
                {
                    for(const std::string &joint : planning_groups_joints.names)
                    {
                        if(joint == joint_name)
                        {
...
//        Frame F_A_C = F_A_B * F_B_C;
    ...
    int j=0;
    double joint_value;
    for(const KDL::Segment &segment : kdl_chain_.segments)
    {
...
void RobotModel::getRobotCollisions(std::vector<urdf::CollisionSharedPtr > &  robotCollisions)
{
    ...
    {
        link_collisions=it->second.getLinkCollisions();
        for(const urdf::CollisionSharedPtr &collision : link_collisions)
        {
           robotCollisions.push_back( collision );
        }
    }
}
...
void RobotModel::getRobotVisuals(std::vector<urdf::VisualSharedPtr > &  robotVisuals)
{
    ...
    for(std::map<std::string, RobotLink> ::iterator it=robot_state_.robot_links_.begin();it!=robot_state_.robot_links_.end();it++)
    {
        ...
        for(const urdf::VisualSharedPtr &visual : link_Visuals)
        {
           robotVisuals.push_back( visual );
        }
    }
}
...
//         std::cout<<kdl_chain_.getSegment(i).getName()<<std::endl;
        planning_group_link_name.push_back(segment.getName());
    }
...
bool RobotModel::getKinematicsSolver(const std::string &name, kinematics_library::AbstractKinematicPtr &kinematic_solver)
{
    if (robot_kinematics_map_.find(name) == robot_kinematics_map_.end())
    {
        LOG_INFO_S<<"[RobotModel]: No kinematic solver is available with name "<<name.c_str();        
        return false;
    }
    
    kinematic_solver = robot_kinematics_map_[name];
    return true;
}
